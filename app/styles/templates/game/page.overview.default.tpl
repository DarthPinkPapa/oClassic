...
<table class="table519">
	...
	<tr>
		<td>{$LNG.Messages}</td>
		<td colspan="2"><a style="color:yellow" href="?page=messages">{$messages}</a></span></td>
	</tr>
	{/if}
	<tr>
...
<table class="table519">
	<tr>
		<th colspan="3">{$LNG.fl_my_planets}</th>
	</tr>
	<tr>
		<td>{$LNG.fl_my_planets}</td>		
		<td colspan="2">
			{if ($planetsCount<$planetsMaxAllowed)}
			<span style="color:yellow">
			{$planetsCount}/{$planetsMaxAllowed} 
			</span>
			{else}
			{$planetsCount}/{$planetsMaxAllowed} 
			{/if}
	...
	{if count($research["queue"])>0}	
	{$q = $research["queue"][0]}	
	<tr>
		<td>{$LNG.lm_research} <div class="timer" data-time="{$q['resttime']}">{pretty_fly_time($q["resttime"])}</div></td>
		<td colspan="2">{$LNG.tech[$q["element"]]} ({$q["level"]}): {$q["display"]}</td>		
	</tr>	
	{/if}
